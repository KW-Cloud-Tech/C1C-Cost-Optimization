...
def get_checks():
    ...
    combined = []
    for account_id in CC_ACCOUNTIDS.split(','):
        CC_PAGESIZE = 1000
        CC_PAGENUMBER = 0
        ...
        while counter <= max_results:
            ...
            if page["meta"]["total"] > 10000:
                raise Exception(
                    "Maximum number of results for Checks API exceeded (10000), please limit results by including additional filters."
                )
            else:
...
def create_csv():
    ...
    if response_json["meta"]["total"] > 10000:
        ...
        with open("wastage-data-{}.csv".format(timestamp), mode="w") as wastage_file:
            ...
            for check in response_json["data"]:
                if check["attributes"]["waste"] > 0 or check["attributes"]["cost"] > 0:
                    wastage_writer.writerow(
                        [
...
try:
    print("Please set the CONFORMITY-APIKEY:")
    CC_APIKEY = input()
except:
    raise Exception(
...
try:
    ...
    print("Please define the CONFORMITY-REGION :")
except:
    CC_REGION = "us-west-2"
